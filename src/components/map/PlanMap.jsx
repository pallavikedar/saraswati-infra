...
// export const FLY_MS = 520;    // the camera closing in on a pick
// export const easeOut = (t) => 1 - (1 - t) ** 3;

// /* The close-up. GUTTER is clear ground kept around the plot for the
//    dimension figures, in METRES — those stand off the edge by a fixed
//    ground distance whatever the plot's size, so a small plot needs a
//    bigger share of the screen than a big one, and a percentage would get
//    it wrong at both ends. SLACK keeps the drawing off the glass.
...
//  *
//  * While a plot is raised the camera turns the map CONTAINER, which has
...
//  * wheel zoom are taken over for the duration.
//  *
//  * A pick also FLIES the camera in: the chosen plot is brought to the
//  * middle of the view and closed in on until the plot and the dimension
//  * figures around it fill the screen. See flyTo.
//  *
...
//      Picking a plot should bring it to you rather than leave you to go
//      find it. Centre is the plot's bounding box, not its centroid: an
...
//      camera goes as close as that allows. Fitting each axis separately
//      rather than to the short side is what makes the close-up close —
//      a wide plot fills the width instead of being framed for a height
//      it doesn't have.
...
//     const availW = Math.max(el.clientWidth - inset.left - inset.right, 160);
//     const availH = Math.max(
//       (el.clientHeight - inset.top - inset.bottom) * Math.cos(MAX_TILT), 160,
//     );

//     const mpp = Math.max(needW / availW, needH / availH);   // metres per pixel wanted
...
//        zoom mid-flight is a staircase instead of a glide. */
//     m.setOptions({ isFractionalZoomEnabled: true });
...
//       const el = viewRef.current;
//       if (!el) return;
//       if (!selRef.current) { setPad({ x: 0, y: 0 }); return; }
...
//     setMap(m);
//     onReady();
//   }, [maps, mapRef, toLL, bounds, onReady]);
...
//   /* Turn the view by dragging, while a plot is raised. With nothing
//      raised the same gesture pans the map underneath. */
//   const startOrbit = (e) => {
//     stopFly();
...
//     camRef.current.spin = g.spin + dx * 0.008;                      // across to turn
//     camRef.current.tilt = clamp(g.tilt - dy * 0.006, 0, MAX_TILT);  // up and down to tip
//     overlayRef.current.draw();
//     bump((n) => n + 1);
//   };

//   const endDrag = () => { touched.current = Date.now(); dragRef.current = null; };
...
//           style={{
//             position: 'absolute', top: 'var(--mh-block)', left: 14, zIndex: 6,
...
// export const DOWN_MS = 190;   // the old plot sinking
// export const UP_MS = 430;     // the new one rising
// export const FLY_MS = 620;    // the camera closing in on a pick
// export const easeOut = (t) => 1 - (1 - t) ** 3;
...
//    wrong at both ends. It is the main lever on how close the camera
//    gets: on a 12 m plot every 0.5 m here is roughly 4% of the frame.
//    Too low and DimensionOverlay's figures clip at the edges.
...
//    pull the camera out to nothing. FLY_MAX_Z must not exceed the map's
//    own maxZoom or the fit is silently clamped and small plots stop
//    getting closer. Past about 21 the satellite imagery has no more
//    detail and Google upscales it: the overlay and its dimensions stay
//    sharp, the photo behind them goes soft. 22 is the honest ceiling if
//    that matters to the client; 24 if legibility of the figures wins. */
// const GUTTER = 0.9;
// const SLACK = 1.02;
// const LIFT_BIAS = 0.5;
// const FLY_MIN_Z = 17;
...
//      plot's box PLUS the gutter its dimension figures live in, and the
//      camera goes as close as that allows. Fitting each axis separately
//      rather than to the short side is what makes the close-up close — a
//      wide plot fills the width instead of being framed for a height it
//      doesn't have.
...
//      nothing to do". The isFractionalZoomEnabled line matters for the
//      same reason: without it raster hybrid rounds the zoom, and a
//      rounded zoom mid-flight is a staircase instead of a glide.
//   --------------------------------------------------------------- */
//   const flyTo = useCallback((name) => {
//     const m = mapRef.current;
//     const el = viewRef.current;
...
//     /* Raster hybrid rounds the zoom unless this is on. Set alongside
//        maxZoom because the fit above can ask for more than the map was
...
//           `top` reads the header's own variable so the two never collide
//           at any breakpoint — see Mapheader.css. */}
